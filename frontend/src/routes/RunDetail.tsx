...
export default function RunDetail() {
  ...
  const telegram = sp.get("view") === "telegram";  // clean embedded view for Telegram
  ...
  if (!run) return telegram
    ? <div className="h-full grid place-items-center text-muted text-sm">…loading…</div>
    : <Page title="Run">…loading…</Page>;
  ...
  // ─────── Clean embedded view for Telegram "View progress" ───────
  // Just the status header + live event timeline, filling the viewport. No
  // sidebar (hidden in App.tsx), no retro/artefacts/graph/input-output chrome.
  if (telegram) {
    ...
    return (
      <div className="flex flex-col h-full bg-bg">
        <header className="flex items-center gap-2 px-4 py-2.5 border-b border-line bg-bg-2 shrink-0">
          <span className="font-mono text-sm text-accent">Run {run.id.slice(0, 8)}</span>
          <StatusBadge status={run.status} />
          <span className="text-xs text-muted ml-auto">
            {events.length} events · {run.tokens_in}/{run.tokens_out} tok
          </span>
          {run.status === "running" && (
            <button className="btn btn-danger btn-sm" data-testid="run-cancel"
                    onClick={async () => { await api.cancelRun(run.id); }}>cancel</button>
          )}
        </header>
        <div className="flex-1 overflow-y-auto px-3 py-2" data-testid="run-events">
          <TimelineList display={display} expanded={expanded} toggle={toggle} runId={run.id} telegram />
          {run.status === "running" && (
            <div className="text-xs text-muted px-1 py-2 animate-pulse">▌ working…</div>
          )}
          {run.error && (
            <pre className="codebox text-err text-xs whitespace-pre-wrap mt-2">{run.error}</pre>
          )}
        </div>
        {outputText && run.status !== "running" && (
          <div className="border-t border-line bg-bg-2 px-4 py-2 max-h-44 overflow-auto shrink-0">
            <div className="text-xs text-muted mb-1">output</div>
            <pre className="codebox text-xs whitespace-pre-wrap">{outputText}</pre>
          </div>
        )}
      </div>
    );
    ...
    <Page title={`Run ${run.id.slice(0, 12)}`}
        ...
        <div className="card mb-4" data-testid="run-thread">
          ...
          {Object.entries(assembled).map(([node, text]) => (
            <div key={node} className="mb-3">
              ...
              <pre className="codebox whitespace-pre-wrap">{text}</pre>
            </div>
          ))}
        </div>
      )}
      ...
      <div className="card" data-testid="run-events">
        ...
        <div className="max-h-[600px] overflow-y-auto space-y-1">
          <TimelineList display={display} expanded={expanded} toggle={toggle} runId={run.id} />
        </div>
      </div>
    </Page>
  );
...
// Shared event-timeline rows — used by both the full RunDetail and the
// clean ?view=telegram embedded view.
function TimelineList({ display, expanded, toggle, runId, telegram }:
    { display: RunEvent[]; expanded: Set<string>; toggle: (id: string) => void; runId: string; telegram?: boolean }) {
  const childSuffix = telegram ? "?view=telegram" : "";
  return (
    <>
      {display.map(e => {
        ...
        return (
          <div key={e.id} className="border-b border-line py-1">
            ...
            {expandedNow && (
              <>
                {(e.payload?.child_run_id || e.payload?.run_id) && (e.payload.child_run_id || e.payload.run_id) !== runId && (
                  <div className="ml-24 mt-1 text-xs">
                    →&nbsp;
                    <Link to={`/runs/${e.payload.child_run_id || e.payload.run_id}${childSuffix}`} className="font-mono">
